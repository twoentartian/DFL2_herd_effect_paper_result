...
folders = ["1_1", "1_4", "1_8", "1_10", "1_12", "1_16"]
# titles = ["random network - 8 peers", "grid", "loop", "star"]
titles = ["train/average interval=1:1", "train/average interval=1:4", "train/average interval=1:8", 
          "train/average interval=1:10", "train/average interval=1:12", "train/average interval=1:16"]

maximum_tick = 10000
save_name = "train_average_interval"
...
if flag_generate_whole:
    ...
    for folder_index in range(len(folders)):
        ...
        weight_diff_axis.legend(ncol=4, prop={'size': 6})
        weight_diff_axis.set_title('Subplot ' + str(folder_index+1) + 'b-layer weight diff: ' + titles[folder_index])
